...
c     convert from nm to cm-1
      WNUMLO = REAL(1.E7 / WVLHI)
      WNUMHI = REAL(1.E7 / WVLLO)
     ...
     &    CALL ERRMSG2('PLKINT--temperature or wavenums. wrong',.TRUE.)
      ...
      V( 1 ) = C2*WNUMLO / REAL(T)
      V( 2 ) = C2*WNUMHI / REAL(T)
...
c                          ** by iterating Simpson rule to convergence.
   ...
   30    CONTINUE
         ...
         PLKINT = SIGDPI * REAL(T)**4 * CONC * VAL
...
c                                    ** WNUMLO and WNUMHI both large
      ...
      PLKINT = SIGDPI * REAL(T)**4 * PLKINT

      IF( PLKINT.EQ.0.0 )
     &    CALL ERRMSG2('PLKINT--returns zero; possible underflow',
     &    .FALSE.)
...
c $Header: /export/home/cvs/bubble/mstrnx/plkint.f,v 1.1.1.1 2009/12/12 15:47:06 bernhard Exp $
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
